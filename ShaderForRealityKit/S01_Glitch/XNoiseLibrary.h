...
float cnoise(float2 P)
{
    ...
    float4 gx = fract(i / 41.0) * 2.0 - 1.0;
...
float3 bcc4_grad(float hash)
{
    ...
    float3 cube = fract(floor(hash / float3(1, 2, 4)) * 0.5) * 4 - 1;
    ...
    float type = fract(floor(hash / 8) * 0.5) * 2;
...
float4 Bcc4NoisePlaneFirst(float3 X)
{
    ...
    float4 result = Bcc4NoiseBase((X * orthonormalMap));
    return float4((orthonormalMap * result.xyz), result.w);
}
